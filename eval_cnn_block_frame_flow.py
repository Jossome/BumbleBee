    ...
    for i in range(len(video_frames)):
        ...
        for i_frame in range(len(frames)):
            ...
            if (i_frame + 1) % 15 == 0:
                ...
                instance_frames = Variable(tensor_frames.unsqueeze(0)).cuda()
                instance_flow_x = Variable(tensor_flow_x.unsqueeze(0)).cuda()
                instance_flow_y = Variable(tensor_flow_y.unsqueeze(0)).cuda()
